...
public class problem_2306 {
    ...
    public long solution(String[] ideas) {
        Set<String>[] dictionary = new HashSet[26];
        for (int i = 0; i < 26; i++) { 
            dictionary[i] = new HashSet<>();
        }
        for (String idea : ideas) {
            char c = idea.charAt(0);
            dictionary[c - 'a'].add(idea.substring(1));
        }
        long count = 0;
        for (int i = 0; i < 25; i++) {
            for (int j = i + 1; j < 26; j++) {
                long sameSuffix = 0;
                for (String suffix : dictionary[i]) {
                    if (dictionary[j].contains(suffix)) {
                        sameSuffix++;
                    }
                }
                count += 2 * (dictionary[i].size() - sameSuffix) * (dictionary[j].size() - sameSuffix);
            }
        }
        return count;
    }
