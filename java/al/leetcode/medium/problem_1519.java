...
public class problem_1519 {
    ...
    public int[] solution(int n, int[][] edges, String labels) {
        int[] trees = new int[n];
        ...
        solution(0, edgeMap, trees, edges, visited, labels);
        return trees;
    }

    public int[] solution(int start, Map<Integer, List<Integer>> edgeMap, int[] trees, int[][] edges, boolean[] visited, String labels) {
        trees[start] = 1;
        visited[start] = true;
        int[] labelMap = new int[26];
        for (int i = 0; i < edgeMap.get(start).size(); i++) {
            int to = edgeMap.get(start).get(i);
            if (!visited[to]) {
                int[] prev = solution(to, edgeMap, trees, edges, visited, labels);
                trees[start] += prev[labels.charAt(start) - 'a'];
                for (int c = 0; c < labelMap.length; c++) {
                    labelMap[c] += prev[c];
                }
            }
        }
        labelMap[labels.charAt(start) - 'a'] = trees[start];
        return labelMap;
    }
